...
const week = uiCopy.today.weekdays.map((day, index) => ({
  day,
  state: index < 2 ? 'done' : index === 2 ? 'today' : 'open',
}))
  ...
  return (
    <div className="today-screen">
      <header className="today-header">
        <div>
          <span className="today-header__date">{uiCopy.today.date}</span>
          <h1>{uiCopy.today.greeting}</h1>
          <p>{uiCopy.today.introduction}</p>
        </div>
      ...
      <section className="workout-card" aria-labelledby="today-workout-title">
        ...
        <div className="workout-card__content">
          <span className="eyebrow">{uiCopy.today.movementLabel}</span>
          <h2 id="today-workout-title">{uiCopy.today.workoutTitle}</h2>
          <p>{uiCopy.today.workoutDescription}</p>
          <div className="workout-card__meta">
            <span>
              <AppIcon name="clock" /> {uiCopy.today.workoutDuration}
            </span>
            <span>{uiCopy.today.impactLevel}</span>
          </div>
          <button type="button" disabled aria-label={uiCopy.today.startWorkoutPreviewLabel}>
            <span>{uiCopy.today.startWorkout}</span>
            <AppIcon name="arrow" />
          </button>
        </div>
      </section>
      ...
      <section className="streak-card" aria-labelledby="streak-title">
        <div className="streak-card__heading">
          <div>
            <span className="eyebrow">{uiCopy.today.rhythmLabel}</span>
            <h2 id="streak-title">{uiCopy.today.streak}</h2>
          </div>
          <div className="sun-mark" aria-hidden="true">
            <span />
          </div>
        </div>
        <div className="week-row" aria-label={uiCopy.today.weeklyActivityPreviewLabel}>
          {week.map((item) => (
      ...
      <div className="today-grid">
        <section className="metric-card nutrition-card" aria-labelledby="nutrition-title">
          <span className="eyebrow">{uiCopy.today.nourishmentLabel}</span>
          <h2 id="nutrition-title">{uiCopy.today.nutritionTitle}</h2>
          <div className="nutrition-stat">
            <span>{uiCopy.today.calories}</span>
          ...
          <div className="nutrition-stat">
            <span>{uiCopy.today.protein}</span>
            <strong>92 {uiCopy.today.grams} <small>/ 125 {uiCopy.today.grams}</small></strong>
        ...
        <section className="metric-card weight-card" aria-labelledby="weight-title">
          <span className="eyebrow">{uiCopy.today.weightTrendLabel}</span>
          <h2 id="weight-title">64.8 <small>{uiCopy.today.kilograms}</small></h2>
          <span className="trend-label">{uiCopy.today.monthlyWeightChange}</span>
      ...
      <p className="today-footer-note">{uiCopy.today.footer}</p>
    </div>
  )
}
